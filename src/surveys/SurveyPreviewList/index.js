...
class Mdl_ColumnsProperties {
  constructor(target, visible, searchable) {
    this.target = target;
...
function SurveyPreviewList() {

	$(document).on("click",".smtbtn",function(){			

			window.open("http://localhost:3000/surveys/SurveyPreview?PageID="+$($(this).parent().parent().children()[2]).text()+"",'_blank');
	});
    ...
    if ($("#tablecontent").length > 0) {
		...
		axios.get('https://localhost:5001/api/GetSurveyResultList', params, { headers })
